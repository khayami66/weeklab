...
> **進捗の正本は `作業工程手順書.md`**（フェーズ別のチェックリスト）。本書は計画の骨格を示す。
> 実装状態の一覧は `要件定義書.md` §5.1 の状態欄を見る。

---

## Step 0: 設計レビュー（2026-04-21 完了）
...
詳細は `grading_design.md` §8。氏名は扱わず学年・クラス・出席番号のみで運用する。

- **G1**：`types/index.ts` に成績層の型を追加、`lib/grading.ts`（純粋関数）＋ Vitest
  - 未受験 `null` の除外、`max === 0` の判定不能、境界値（90%/60%）、転出の除外を必ずテスト
- **G2**：`/settings` に評定閾値・テストマスタの2セクション追加（在籍人数の設定は持たない）
- **G3**：`/grades` タブ1（得点入力）
- **G4**：`/grades` タブ2（学期評定）・タブ3（観点の偏り）

**確認**：実データで9月の成績処理が最後まで回り、校務支援システムへ転記できる。
